...
warp() {
    ...
    if [ -f "${prefix}Affine.txt" -a -f "${prefix}Warp.nii.gz" ]; then
        log_success "Made non-linear warp: '${prefix}Affine.txt', '${prefix}Warp.nii.gz'"
    else
        log_error "'${prefix}Affine.txt' and/or '${prefix}Warp.nii.gz' were not generated by ANTS script 'antsIntroduction.sh'"
        exit 1
    fi
}
...
run "unu resample -k cheap -s $new_size -i wmparc-in-bse-1mm.nrrd | unu save -e gzip -f nrrd -o wmparc-in-bse_unu.nrrd"
run "ResampleImage 3 wmparc-in-bse-1mm.nrrd wmparc-in-bse_resample.nrrd 3x3x3 nn"
ConvertBetweenFileFormats wmparc-in-bse_unu.nrrd wmparc-in-bse_unu.nrrd short
ConvertBetweenFileFormats wmparc-in-bse_resample.nrrd wmparc-in-bse_resample.nrrd short
